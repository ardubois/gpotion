...
def ecalc_nbodies(j,i,p,softening,fx,fy,fz) do
  ...
  def nbodies([],_p_copy,_dt,_softening,_n) do
    []
  end
  def nbodies(p,p_copy,dt,softening,n) do
    #p=nbodies(i-1,p,dt,softening,n)
    ...
    resp=nbodies(rest_p,p_copy,dt,softening,n)
    [x,y,z,vx,vy,vz]++resp
  end
...
def cpu_integrate([],_dt) do
  []
end
def cpu_integrate(p, dt) do
  [x,y,z,vx,vy,vz]= Enum.take(p,6)
  rest_p = Enum.drop(p,6)
  x = x + vx * dt
  y = y + vy * dt
  z = z + vz * dt
  result=cpu_integrate(rest_p,dt)
